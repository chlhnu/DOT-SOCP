...
function fR = interpolate_tStagger(f)
    ...
    fR(oddY, oddX, oddT) = f; % t - nearest
    fR(oddY, oddX, evenT) = f; % t - nearest
    fR(evenY, oddX, :) = interpolate_linear(fR(oddY, oddX, :), 1); % y - linear
    fR(:, evenX, :) = interpolate_linear(fR(:, oddX, :), 2); % x - linear
end
...
function phiR = interpolate_phi(phi, ny, nx, nt)
    ...
    phiR(evenY, oddX , oddT ) = interpolate_linear(phiR(oddY, oddX, oddT), 1); % y - linear
    phiR(:    , evenX, oddT ) = interpolate_linear(phiR(:   , oddX, oddT), 2); % x - linear
    phiR(:    , :    , evenT) = interpolate_linear(phiR(:   , :   , oddT), 3); % t - linear
