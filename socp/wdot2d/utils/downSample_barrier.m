function weightR = downSample_barrier(nt, nx, ny, weight)
%% Calculate the downsampling of "weight" located on a staggered grid
...
log_weightR = cat(1, ...
    RestriT * log_weight(1 : bxInd-1), ...
    RestriX * log_weight(bxInd : byInd-1), ...
    RestriY * log_weight(byInd : end));
weightR = power(exp(1), log_weightR);
